...
`proxy.js|ts` 파일은 [Proxy](/pages-router/getting-started/16-proxy.md)를 작성하고 요청이 완료되기 전에 서버에서 코드를 실행하는 데 사용됩니다. 들어오는 요청을 기반으로 응답을 수정하거나 요청을 리다이렉트하고, 요청 또는 응답 헤더를 수정하거나 직접 응답할 수 있습니다.
...
프로젝트 루트 또는 `src` 내부(해당되는 경우)에 `proxy.ts` (또는 `.js`) 파일을 생성하여 `pages`와 같은 수준에 위치하도록 하세요.
...
export function proxy(request: NextRequest) {
  ...
  // Proxy에서 설정한 헤더를 getServerSideProps에서 사용 가능
...
- [NextRequest](/pages-router/api-reference/functions/next-request-response.md) - NextRequest의 API 참조
- [NextResponse](/pages-router/api-reference/functions/next-request-response.md) - NextResponse의 API 참조
- [Proxy 가이드](/pages-router/getting-started/16-proxy.md) - Proxy 사용 가이드
