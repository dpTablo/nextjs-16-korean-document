...
export function onRequestError(
  ...
  context: {
    ...
    routePath: string               // 라우트 파일 경로, 예: /pages/blog/[slug]
...
## Pages Router에서의 특별 고려사항

Pages Router를 사용하는 경우, `context.routerKind`는 항상 `'Pages Router'` 값을 가지며, `context.routePath`는 `pages/` 디렉토리 내의 파일 경로를 나타냅니다.

```ts
export async function onRequestError(error, request, context) {
  if (context.routerKind === 'Pages Router') {
    console.log('Pages Router 오류:', {
      page: context.routePath, // 예: /pages/blog/[slug].tsx
      type: context.routeType,  // 'render' 또는 'route' (API Routes)
    })
  }
}
```
