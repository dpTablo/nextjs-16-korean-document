# Next.js 16.1.4 한국어 번역 상태

## 📊 전체 번역 현황

**총 번역 완료:** 247개 / 약 250개 문서 (약 99%)

---

## ✅ 완료된 섹션

### 1. App Router - Getting Started (18/18 - 100%)

모든 기본 시작 문서가 번역되었습니다.

- [x] Installation (설치)
- [x] Project Structure (프로젝트 구조)
- [x] Layouts and Pages (레이아웃과 페이지)
- [x] Linking and Navigating (링크 및 네비게이션)
- [x] Server and Client Components (서버 및 클라이언트 컴포넌트)
- [x] Cache Components (캐시 컴포넌트) 🆕
- [x] Caching and Revalidating (캐싱 및 재검증)
- [x] Fetching Data (데이터 가져오기)
- [x] Updating Data (데이터 업데이트)
- [x] Error Handling (에러 처리)
- [x] CSS (CSS 스타일링)
- [x] Image Optimization (이미지 최적화)
- [x] Font Optimization (폰트 최적화)
- [x] Metadata and OG Images (메타데이터와 OG 이미지)
- [x] Route Handlers (라우트 핸들러)
- [x] Proxy (프록시)
- [x] Deploying (배포)
- [x] Upgrading (업그레이드) 🆕

### 2. App Router - Guides (48/48 - 100% 완료!)

모든 가이드 문서가 번역되었습니다.

**✅ 완료 (25개):**
- [x] Forms and Mutations (폼 처리)
- [x] Authentication (인증)
- [x] Environment Variables (환경 변수)
- [x] Testing - Jest
- [x] Testing - Playwright
- [x] Internationalization (국제화)
- [x] Caching (캐싱)
- [x] Self-Hosting (셀프 호스팅)
- [x] Static Exports (정적 내보내기)
- [x] Debugging (디버깅)
- [x] Data Fetching Patterns (데이터 페칭 패턴)
- [x] Server Actions Patterns (Server Actions 패턴)
- [x] Rendering (렌더링 전략)
- [x] Tailwind CSS v3
- [x] Analytics
- [x] Lazy Loading
- [x] MDX
- [x] Third Party Libraries
- [x] Prefetching
- [x] TypeScript
- [x] Content Security Policy
- [x] Draft Mode
- [x] OpenTelemetry
- [x] CSS-in-JS
- [x] Sass

**✅ 추가 완료 (4개):**
- [x] ISR (Incremental Static Regeneration)
- [x] PWAs (Progressive Web Applications)
- [x] Redirecting
- [x] Scripts

**✅ 추가 완료 (4개 - Phase 16):**
- [x] JSON-LD (SEO)
- [x] SPAs (Single-Page Applications)
- [x] Custom Server
- [x] Package Bundling

**✅ 추가 완료 (4개 - Phase 17):**
- [x] Videos
- [x] Memory Usage
- [x] CI Build Caching
- [x] Multi-zones

**✅ 추가 완료 (7개 - Phase 18):**
- [x] Data Security (데이터 보안) 🆕
- [x] Local Development (로컬 개발 환경) 🆕
- [x] Next.js MCP Server 🆕
- [x] Migrating (마이그레이션 - 3개 하위 문서 포함) 🆕
- [x] Multi-tenant (멀티테넌트) 🆕
- [x] Upgrading (업그레이드 - codemods, version-16 포함) 🆕
- [x] Backend for Frontend 🆕

**✅ 추가 완료 (4개 - Phase 31):**
- [x] Testing - Cypress 🆕
- [x] Testing - Vitest 🆕
- [x] Instrumentation 🆕
- [x] Production Checklist (프로덕션 체크리스트) 🆕

### 3. App Router - API Reference (72/80+ - 90%)

**✅ 완료:**

**지시어 (5/5 - 100%):**
- [x] use client
- [x] use server
- [x] use cache ✅
- [x] use cache: private ✅ 🆕
- [x] use cache: remote ✅ 🆕

**컴포넌트 (5/5 - 100%):**
- [x] Link
- [x] Image
- [x] Script ✅
- [x] Font ✅
- [x] Form ✅ 🆕

**함수 (34/37 - 92%):**
- [x] generateMetadata
- [x] generateStaticParams
- [x] redirect
- [x] notFound
- [x] cookies
- [x] headers
- [x] revalidatePath
- [x] revalidateTag
- [x] permanentRedirect
- [x] useRouter
- [x] usePathname
- [x] useSearchParams
- [x] useParams
- [x] draftMode
- [x] fetch
- [x] NextRequest/NextResponse
- [x] unstable_cache
- [x] generateViewport
- [x] useSelectedLayoutSegment
- [x] useSelectedLayoutSegments
- [x] useReportWebVitals
- [x] unstable_noStore
- [x] userAgent
- [x] connection
- [x] after
- [x] generateImageMetadata
- [x] ImageResponse ✅
- [x] generateSitemaps ✅

**React 19 Hooks (3/3):**
- [x] useFormStatus ✅
- [x] useActionState ✅
- [x] useOptimistic ✅

**추가 완료 함수 (3개 - Phase 19):**
- [x] forbidden ✅
- [x] unauthorized ✅
- [x] unstable_rethrow ✅

**추가 완료 함수 (5개 - Phase 26):**
- [x] cacheLife ✅ 🆕
- [x] cacheTag ✅ 🆕
- [x] refresh ✅ 🆕
- [x] updateTag ✅ 🆕
- [x] useLinkStatus ✅ 🆕

**CLI (2/2 - 100%):**
- [x] create-next-app ✅ 🆕
- [x] next CLI ✅ 🆕

**Edge Runtime (1/1 - 100%):**
- [x] Edge Runtime API ✅ 🆕

**Turbopack (1/1 - 100%):**
- [x] Turbopack API ✅ 🆕

**파일 규칙 (28/28 - 100%):**
- [x] page.js
- [x] layout.js
- [x] template.js
- [x] loading.js
- [x] error.js
- [x] not-found.js
- [x] route.js
- [x] middleware.js
- [x] global-error.js
- [x] default.js
- [x] instrumentation.js
- [x] instrumentation-client.js 🆕
- [x] forbidden.js 🆕
- [x] unauthorized.js 🆕
- [x] mdx-components.js 🆕
- [x] opengraph-image, twitter-image
- [x] sitemap.xml
- [x] robots.txt
- [x] icon, apple-icon, favicon
- [x] manifest.json
- [x] route segment config ✅
- [x] dynamic-routes (동적 라우트)
- [x] parallel-routes (병렬 라우트)
- [x] route-groups (라우트 그룹)
- [x] intercepting-routes (인터셉팅 라우트)
- [x] proxy.js (프록시)
- [x] public folder 🆕
- [x] src folder 🆕

**next.config.js 옵션 (58/60 - 97%):**
- [x] allowedDevOrigins ✅
- [x] appDir ✅
- [x] assetPrefix ✅
- [x] authInterrupts ✅
- [x] basePath ✅
- [x] browserDebugInfoInTerminal ✅ 🆕
- [x] cacheComponents ✅
- [x] cacheHandlers ✅
- [x] cacheLife ✅
- [x] compress ✅
- [x] crossOrigin ✅
- [x] cssChunking ✅
- [x] devIndicators ✅
- [x] distDir ✅
- [x] env ✅
- [x] experimental ✅
- [x] expireTime ✅
- [x] exportPathMap ✅
- [x] generateBuildId ✅
- [x] generateEtags ✅
- [x] headers ✅
- [x] htmlLimitedBots ✅
- [x] httpAgentOptions ✅
- [x] images ✅
- [x] inlineCss ✅
- [x] isolatedDevBuild ✅ 🆕
- [x] logging ✅
- [x] mdxRs ✅
- [x] onDemandEntries ✅
- [x] optimizePackageImports ✅
- [x] output ✅
- [x] pageExtensions ✅
- [x] poweredByHeader ✅
- [x] productionBrowserSourceMaps ✅
- [x] proxyClientMaxBodySize ✅ 🆕
- [x] reactCompiler ✅
- [x] reactMaxHeadersLength ✅ 🆕
- [x] reactStrictMode ✅
- [x] redirects ✅
- [x] rewrites ✅
- [x] sassOptions ✅
- [x] serverActions ✅
- [x] serverComponentsHmrCache ✅
- [x] serverExternalPackages ✅
- [x] staleTimes ✅
- [x] staticGeneration ✅
- [x] taint ✅
- [x] trailingSlash ✅
- [x] transpilePackages ✅
- [x] turbopack ✅
- [x] turbopackFileSystemCache ✅ 🆕
- [x] typedRoutes ✅
- [x] typescript ✅
- [x] urlImports ✅
- [x] useLightningcss ✅
- [x] viewTransition ✅
- [x] webpack ✅
- [x] webVitalsAttribution ✅

---

## ❌ 전체 미완료 섹션

### 4. Pages Router (52/60+ - 87%)

Pages Router 번역이 크게 진행되었습니다.

**✅ API Reference - Functions (5개):**
- [x] getStaticProps ✅
- [x] getStaticPaths ✅
- [x] getServerSideProps ✅
- [x] getInitialProps ✅
- [x] useRouter ✅

**✅ API Reference - Components (4개):**
- [x] Link ✅
- [x] Head ✅
- [x] Image ✅
- [x] Script ✅

**✅ API Reference - File Conventions (5개):**
- [x] _app.js (Custom App) ✅
- [x] _document.js (Custom Document) ✅
- [x] _error.js (Custom Error) ✅
- [x] 404.js ✅
- [x] 500.js ✅

**✅ Building Your Application - Routing (8개):**
- [x] API Routes ✅
- [x] Dynamic Routes ✅
- [x] Pages and Layouts ✅
- [x] Linking and Navigating ✅
- [x] Custom Error ✅ 🆕
- [x] Redirecting ✅ 🆕
- [x] Internationalization ✅ 🆕
- [x] Middleware ✅ 🆕

**✅ Building Your Application - Data Fetching (2개):**
- [x] Client-side Data Fetching ✅
- [x] Forms and Mutations ✅ 🆕

**✅ Building Your Application - Authentication (1개 - Phase 32):**
- [x] Authentication ✅ 🆕

**✅ Building Your Application - Rendering (5개 - Phase 30):**
- [x] Server-side Rendering ✅
- [x] Static Site Generation ✅
- [x] Client-side Rendering ✅
- [x] Automatic Static Optimization ✅
- [x] Edge and Node.js Runtimes ✅

**✅ Building Your Application - Styling (4개 - Phase 30):**
- [x] CSS Modules ✅
- [x] Tailwind CSS ✅
- [x] CSS-in-JS ✅
- [x] Sass ✅

**✅ Building Your Application - Optimizing (5개 - Phase 30):**
- [x] Fonts ✅
- [x] Scripts ✅
- [x] Static Assets ✅
- [x] Lazy Loading ✅
- [x] Analytics ✅

**✅ Building Your Application - Configuring (5개 - Phase 30):**
- [x] Environment Variables ✅
- [x] ESLint ✅
- [x] TypeScript ✅
- [x] Absolute Imports ✅
- [x] PostCSS ✅

**✅ Building Your Application - Testing (4개):**
- [x] Cypress ✅
- [x] Jest ✅
- [x] Playwright ✅
- [x] Vitest ✅

**✅ Building Your Application - Deploying (4개):**
- [x] Deploying ✅
- [x] CI Build Caching ✅
- [x] Multi-zones ✅
- [x] Static Exports ✅

**API Reference - 미완료 (약 8개):**
- [ ] next/server
- 그 외...

### 5. Architecture (5/5 - 100% 완료!)

아키텍처 이해를 위한 문서들입니다.

- [x] Accessibility (접근성) ✅
- [x] Fast Refresh (빠른 새로고침) ✅
- [x] Next.js Compiler (컴파일러) ✅
- [x] Supported Browsers (지원 브라우저) ✅
- [x] Turbopack (번들러) ✅

### 6. Community (2/2 - 100% 완료!)

커뮤니티 관련 문서입니다.

- [x] Community Index (커뮤니티 개요) ✅ 🆕
- [x] Contribution Guide (기여 가이드) ✅ 🆕

---

## 🎯 우선순위별 미완료 작업

### 🔴 높은 우선순위 (가장 자주 사용)

**App Router - API Reference - 함수:**
- [ ] useRouter - 클라이언트 사이드 라우팅
- [ ] usePathname - 현재 경로 가져오기
- [ ] useSearchParams - URL 쿼리 파라미터
- [ ] permanentRedirect - 영구 리디렉션
- [ ] fetch - 데이터 페칭 (Next.js 확장)
- [ ] draftMode - 초안 모드

**App Router - API Reference - 컴포넌트:**
- [ ] Script - 스크립트 최적화
- [ ] Font - 폰트 최적화 컴포넌트
- [ ] Form - 폼 컴포넌트

**App Router - API Reference - 파일 규칙:**
- [ ] page.js - 페이지 파일 규칙
- [ ] layout.js - 레이아웃 파일 규칙
- [ ] loading.js - 로딩 UI
- [ ] error.js - 에러 처리
- [ ] not-found.js - 404 페이지
- [ ] route.js - API 라우트
- [ ] middleware.js - 미들웨어

**App Router - Guides:**
- [ ] Data Fetching and Caching - 데이터 페칭 심화
- [ ] Server Actions and Mutations - Server Actions 심화
- [ ] Rendering - 렌더링 전략
- [ ] Optimizing - 성능 최적화
- [ ] TypeScript - TypeScript 사용

### 🟡 중간 우선순위

**App Router - API Reference:**
- [ ] next.config.js 주요 옵션들 (images, redirects, rewrites, headers 등)
- [ ] Route Segment Config (dynamic, revalidate 등)
- [ ] Metadata Files (sitemap, robots, manifest)
- [ ] 나머지 Hook 함수들 (useParams, useSelectedLayoutSegment 등)

**App Router - Guides:**
- [ ] Styling - 스타일링 심화
- [ ] Configuring - 설정 심화
- [ ] Analytics - 분석 도구
- [ ] SEO - 검색 엔진 최적화
- [ ] Security - 보안
- [ ] Performance - 성능

### 🟢 낮은 우선순위

**Pages Router:**
- 전체 Pages Router 문서 (App Router가 우선)

**Architecture:**
- 아키텍처 이해 문서들

**Community:**
- 커뮤니티 문서들

---

## 📈 권장 다음 번역 순서

### Phase 4: 핵심 API Reference 완성 (약 15개)

1. **Hook 함수 (7개):**
   - useRouter
   - usePathname
   - useSearchParams
   - useParams
   - permanentRedirect
   - draftMode
   - fetch (Next.js 확장)

2. **컴포넌트 (3개):**
   - Script
   - Font
   - Form

3. **파일 규칙 (5개):**
   - page.js
   - layout.js
   - loading.js
   - error.js
   - route.js

### Phase 5: 고급 가이드 (약 10개)

1. Data Fetching and Caching (심화)
2. Server Actions and Mutations (심화)
3. Rendering Strategies
4. TypeScript
5. Optimizing
6. Styling (심화)
7. SEO
8. Analytics
9. Security
10. Performance

### Phase 6: next.config.js 옵션 (약 20개 핵심)

주요 설정 옵션들:
- images
- redirects
- rewrites
- headers
- env
- typescript
- webpack
- experimental 옵션들

### Phase 7: Pages Router (선택사항)

App Router가 우선이므로 필요시 진행

---

## 💡 번역 전략 제안

### 1. 사용 빈도 기반
가장 자주 사용되는 API부터 번역하여 실용성 극대화

### 2. 완성도 기반
한 섹션을 완전히 마무리한 후 다음 섹션으로 이동

### 3. 의존성 기반
서로 관련된 문서들을 함께 번역하여 일관성 유지

### 4. 점진적 확장
핵심 → 고급 → 심화 순서로 점진적 확장

---

**현재 상태:** Phase 32 완료 (244개 문서, 98% 달성!)
**이번 Phase 32 성과:**
- App Router Getting Started 2개 문서 번역 완료 ✅
  - Cache Components (캐시 컴포넌트) 🆕
  - Upgrading (업그레이드) 🆕
- App Router File Conventions 6개 문서 번역 완료 ✅
  - forbidden.js (접근 금지 페이지) 🆕
  - unauthorized.js (미인증 페이지) 🆕
  - instrumentation-client.js (클라이언트 인스트루멘테이션) 🆕
  - mdx-components.js (MDX 컴포넌트) 🆕
  - public folder (public 폴더) 🆕
  - src folder (src 폴더) 🆕
- next.config.js 옵션 5개 번역 완료 ✅
  - reactMaxHeadersLength 🆕
  - proxyClientMaxBodySize 🆕
  - isolatedDevBuild 🆕
  - browserDebugInfoInTerminal 🆕
  - turbopackFileSystemCache 🆕
- Pages Router 6개 문서 번역 완료 ✅
  - Routing - Custom Error (커스텀 에러) 🆕
  - Routing - Redirecting (리다이렉트) 🆕
  - Routing - Internationalization (국제화) 🆕
  - Routing - Middleware (미들웨어) 🆕
  - Data Fetching - Forms and Mutations (폼과 데이터 변경) 🆕
  - Authentication (인증) 🆕

**이전 Phase 성과:**
- Phase 31: App Router Getting Started 4개, Guides 4개, File Conventions 5개 번역 완료
- Phase 30: Pages Router Building Your Application 19개 문서 번역 완료 (Rendering, Styling, Optimizing, Configuring)
- Phase 29: Pages Router API Reference 3개 + Routing 4개 + Data Fetching 1개 번역 완료
- Phase 28: Pages Router Components 4개 문서 번역 완료 (Link, Head, Image, Script)
- Phase 27: Pages Router API Reference 5개 문서 번역 완료 (getStaticProps, getStaticPaths, getServerSideProps, _app.js, _document.js)
- Phase 26: API Reference 12개 문서 번역 완료 (Form, cacheLife, cacheTag, refresh, updateTag, useLinkStatus, use cache: private/remote, CLI, Edge Runtime, Turbopack)
- Phase 25: Architecture 섹션 5개 문서 번역 완료
- Phase 20-24: next.config.js 옵션 34개 번역 완료
- Phase 18-19: Guides 7개, API Reference 함수 3개 번역 완료

**다음 목표:** 남은 문서 완료 및 검토
**최종 목표:** 전체 문서 번역 완료 - 99% 달성! 🎉
